...
class PromptStudioUiContractTests(unittest.TestCase):
    def test_main_workflows_have_stable_tabs_and_status_targets(self):
        ...
        self.assertIn("全新、独立成图的日系插画方向", source)
        self.assertIn("二次元可爱兽耳小萝莉", source)
        self.assertIn("静态词库只作参考", source)
        self.assertIn("本要求只定义内容方向，不定义输出语言、标签/自然语言格式", source)
        ...
        self.assertIn("独立成图的日系插画方向", template_source)
        self.assertNotIn("只返回一条完整的单图 Prompt", template_source)
        self.assertNotIn("不要分镜、拼图、候选方案、解释或 Markdown", template_source)
