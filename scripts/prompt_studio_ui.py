...
固定基调：可爱、幼态外观、精致大眼、柔和表情、兽耳或动物耳朵、蓬松尾巴、完整得体的服装与配饰、自然姿态。角色可以拥有不同毛色、发色、瞳色和兽耳类型。每条结果只描写一个可见角色，并且必须是一张图里的一个完整瞬间；必须写清楚主体外观与服装、动作和表情、具体地点、关键道具、人与环境或道具的关系、前景中景背景层次、镜头景别与视角、时间或天气、现场光照和主要色彩。

禁止输出分镜、拼图、多面板、连续镜头、角色设定表或多个候选方案。

可以参考系统提供的静态词库组合角色细节、服饰、道具和环境，但只能选择彼此协调的元素，不要机械堆砌词条，也不要加入与画面无关的元素。静态词库仅作参考，不是必须使用的清单。

原始 Prompt 中的 LoRA、权重、角色身份、标签和内容限制由 Forge 自动保留。不要复述或重写这些基底内容，不要新增画师名、工作室名、作品名、艺术风格词、渲染媒介词、美学标签或质量宣传词。
