...
- `自动生图循环` 可串联 LLM 生成、Forge 原生 txt2img / img2img 生图和下一轮 Prompt，支持限定次数或持续运行直到取消。
...
- 单条失败后自动重试 `0-3` 次；例如设置为 `2` 时，每条最多请求 3 次。
- `重试耗尽后跳过并继续`：开启时记录错误并继续后续 Tag；关闭时停止任务，并把后续 Tag 标记为未处理。
- 批量手动备用评分；开启 LLM 自动评分时，每条成功结果会单独评价，备用评分不参与高分 RAG。
...
7. 点击 `手动重试所选`。手动重试会绕过“已有缓存跳过”规则；成功项会从汇总移除，仍失败项和未勾选项会保留。
8. 需要停止时点击 `取消批量任务`。

队列最多解析 10000 条有效输入；预览表最多显示前 200 条。批量生成每积累 10 条结果或到达队尾时进行事务提交。

同步 HTTP 请求无法在传输中强制终止。取消会在当前 LLM 请求返回或超时后生效，已经提交的结果会保留，尚未处理的 Tag 会进入问题汇总供手动重试。取消按钮只作用于当前浏览器会话发起的批量任务，不会取消其他会话的任务。
...
在 `批处理 > 自动生图循环` 中填写每轮创作要求，插件会按以下顺序执行：

1. 调用当前生成页的 LLM 设置生成一条 Prompt。
2. 将结果覆盖或追加到 Forge 原生 `txt2img` / `img2img` 正向 Prompt。
3. 点击对应的 Forge 原生生成按钮，等待本轮完成后再进入下一轮。

`循环次数` 用于有限批次；勾选 `持续运行直到取消` 后会持续生成，并自动在每轮要求中加入轮次变化提示。`取消自动循环` 会停止后续轮次，并保留已经完成的结果。该流程由浏览器前端串联，因此不要在运行期间关闭页面或切换到会刷新 WebUI 的页面；所有模型、采样器、尺寸、批量大小和其他生图参数都沿用 Forge 当前设置。
...
- 自动评分会为每条需要缓存的生成结果增加一次 LLM 请求，因此批量任务的请求量和费用通常接近翻倍。
