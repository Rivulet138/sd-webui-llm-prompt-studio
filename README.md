...
- `生成并入队`用于手动建立队列；`LLM 无限生成`使用勾选开关启用，`停止`可终止当前 LLM 请求。浏览器队列和服务端队列位于批处理页的高级队列区域。
...
LLM 无限生成会读取当前固定正面 Prompt 作为源内容，并提前准备下一条结果。勾选后点击一次 Forge 生成，扩展会自行逐轮等待 LLM 和 Forge，不依赖修改 Forge 核心文件。每轮只在提交请求的瞬间临时写入生成 Prompt，随后立即恢复页面中的固定正面 Prompt。点击 Forge 中断或取消勾选会停止后续轮次。系统保留最近一批结果，提取重复出现的动作、物品和场景概念形成多样性参考，过于接近时最多重新生成三次。

批处理页的“持续生成并生图”使用明确的 `1-100` 轮，不接受 `0` 作为无限值；需要持续到手动中断时使用 txt2img 内嵌面板的“LLM 无限生成”。
