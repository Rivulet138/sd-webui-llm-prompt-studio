...
`生成提示词` 只发送一次生成请求。开启缓存后，结果写入本地 SQLite；不会追加评分请求。
...
上一轮的 A 不会进入下一轮，也不会无限累积。覆盖模式则每轮仅写入当前新 Prompt。
...
- LLM 请求开始后无法中断底层 HTTP 连接；取消会在当前请求返回或超时后生效。
