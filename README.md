...
- 接收 Ranbooru 当前缓存记录的实时结构化交接，可直接载入生成页，或使用已保存的 LLM 参数处理并按“未评分”缓存。
- Ranbooru 风格标签处理：移除不良标签、自定义通配排除、去重、随机打乱、下划线转空格和最大标签数量。
- 独立的批量缓存页面，支持 LLM 批量生成、直接批量导入、队列预览、单次请求、错误后跳过、问题汇总、勾选后明确重新提交、取消和进度记录。
- `自动生图循环` 将 Prompt 批量生成与 Forge 原生 txt2img / img2img 生图合并在同一面板，可先保存队列再确认生图，也可生成后立即生图。
...
内嵌面板不重复保存连接或工作参数。Provider、URL、模型 ID、凭据和工作参数统一从独立工作室的 `连接设置` 与 `保存全部工作参数` 读取。
...
| 批量任务 | 跳过已有输入和错误后继续；每条去重输入只请求一次，结果统一为未评分 |
| WD14 | API 地址、模型、阈值 |
| 静态词库 | 当前词库目录 |
...
- 批量结果统一按 `0 分 / unrated` 保存，不发送额外评分请求，也不会自动进入高分 RAG；需要时在缓存库中明确选择并执行 LLM 评分。
...
在 `批处理 > LLM 批量生成 > Forge Prompt 队列与自动生图` 中按每行一条填写批量创作要求。重复行会在请求前去重，可选择两阶段检查或一步执行：

1. 点击 `生成到生图队列`，每条唯一输入调用当前 LLM 设置一次并生成逐条 Prompt。检查队列后，点击 `投入已有队列生图`，此阶段不再调用 LLM。
2. 点击 `生成并立即生图` 会只处理本轮新生成的队列项，不会夹带历史待处理队列。每条 Prompt 依次写入 Forge 原生 `txt2img` / `img2img`，确认本轮生图完成后再进入下一条。
3. 开启 `持续生成并生图` 后，`循环轮数` 为正数时执行指定轮数，为 `0` 时持续执行到点击取消。持续模式是显式重复流程，每轮仍只对每条输入请求一次；普通批量模式继续阻止相同请求跨刷新重复提交。
4. 可选 `同时缓存到 SQLite Prompt 库`。每个不同生成结果独立缓存；自动队列和 Ranbooru 实时交接的缓存均为 `0 分 / unrated`，不会隐式执行质量评分。

队列使用带请求 ID 和批次 ID 的版本化浏览器本地存储，刷新页面后仍可恢复，并阻止同一请求跨刷新重复提交；持续模式中不同轮次即使生成相同 Prompt，也会保留为不同队列项。运行中的标签页会持有队列租约，其他标签页不会覆盖或重复执行该队列；`清空队列` 会同时清除去重身份。若浏览器拒绝写入，内存队列仍可继续使用，界面会明确标记“未持久化”。取消只作用于当前阶段和本轮捕获的 Forge 目标，晚到的旧 LLM 结果不会写入新任务。追加模式始终使用启动生图时的基础 Prompt 加当前队列项，不会把前一项累积到后一项。所有模型、采样器、尺寸、批量大小和其他生图参数都沿用 Forge 当前设置。
...
重复发送同一个 Ranbooru 源记录会更新同一条交接记录，不会无限增加重复项；内容与操作均未变化且已经完成时会直接复用结果，不会再次请求 LLM。每次内容变化都会递增交接版本，处理中旧版本返回的结果无法覆盖、缓存或删除新版本。LLM 结果使用独立的 Ranbooru 来源标识，不会与只读同步得到的原始 Tag / 自然语言记录互相覆盖。Ranbooru 站点 `score` 只作为来源元数据；只有本插件 LLM 评价成功产生的评分才能进入高分 RAG。
