...
- `多样化灵感`：用于没有灵感或希望探索不同题材的场景。系统从静态词库按类别抽样，并结合随机化的动作、物品、背景、镜头和光线方向；已有角色 Tag 会被保留，其余视觉要素由模型补全。每条输入都会使用不同的抽样上下文，避免批量结果同质化。
- `按原意转换`：只转换表达方式和目标模型格式，尽量保留原始 Prompt 的画面事实。适合把旧标签转换成 Krea 2、Anima 或自然语言，不重新设计构图。
